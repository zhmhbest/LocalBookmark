 ...
 * @returns {number}
 */
function randomInteger(num: number|[number,number], numMax?: number) {
    /**
    ...
    switch (arguments.length) {
        ...
        case 2:
            return randInt(num as number, numMax + 1);
        default:
            return 0;
    }
}
 ...
 * }
 */
function makeRGBAColor(m: string, r: number, g: number, b: number, a: number) {
    var buffer = [];
    buffer.push(m);
    buffer.push('(');
 ...
 * @returns {string}
 */
function randomColor(params: {mode?: string, rr?: [number, number], gg?: [number, number], bb?: [number, number], aa?: [number, number], dev?: [number, number]}) {
...
export function randomSoftRGBColor() {
    return randomColor({
        rr: [30, 200],
        gg: [20, 150],
        bb: [20, 220],
